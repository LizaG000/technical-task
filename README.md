Техническое задание
Стек FastApi, Postgres, Redis

Связь таблиц - один ко многим.
Одна запись в role, много записей в permission.
Одна запись в elements, много записей в permission.
role <---- permission ----> elements

Структуру таблиц можно посмотреть в src.infra.postgres.table.py

В таблице roles хранятся роли, например, user, admin, которые мы присваиваем субъектам (пользователям).

В таблице elements хранятся ресурсы, к которым будут применяться права доступа по ролям.

В таблице permission хранятся права доступа к ресурсам по ролям. В данной реализации права выдаются на create, get, get_all, patch, delete.

Для проверки прав доступа делается запрос в БД (src.infra.postgres.gateways.roles), с помощью которого ищется роль у пользователя, которая может предоставить необходимые права доступа. Если такая роль не находится, то возвращается ошибка 403.
